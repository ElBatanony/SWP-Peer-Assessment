...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.db.collection("submissions").doc(this.assignmentId + "-" + this.userId).get().then(function (doc) {
                if (doc.exists) {
            ...
            this.db.collection("assessments").where("submissionId", "==", this.assignmentId + '-' + this.userId).get().then(function (querySnapshot) {
                var subm = document.getElementById("reviews");
                querySnapshot.forEach(function (doc) {
                    subm.innerHTML += "<a href='/assessments/" + doc.id + "'>Review by " + doc.data().author + "</a><br>"
                })}).catch(function (error) {
                    console.log("Error getting document:", error);
                });
        }
